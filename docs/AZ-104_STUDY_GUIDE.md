...
### Microsoft Entra ID ライセンス管理

Entra ID Premium P1/P2 などの有償機能を利用するには、ユーザーまたはグループにライセンスを割り当てる必要があります。

#### ライセンス割り当ての方式

| 方式 | 説明 | 用途 |
|------|------|------|
| **直接割り当て (Direct)** | 個々のユーザーに手動でライセンスを付与 | 少人数・特例ユーザー |
| **グループベースライセンス (Group-based licensing)** | セキュリティグループにライセンスを割り当てると、メンバーへ自動的に継承 | 部署・役職単位の大規模管理 |
| **継承 (Inherited)** | 親グループからの継承表示。Direct と Inherited が併存可能 | 階層的な権限設計 |

#### グループベースライセンスのポイント

```
┌────────────────────────────────────────────────────────┐
│ グループに Entra ID P1 を割り当て                      │
│   ↓                                                    │
│ メンバー全員に自動で付与（追加/削除がリアルタイム反映）│
│                                                        │
│ ライセンス不足時:                                      │
│   - 新規追加メンバーは「ライセンスエラー」状態になる   │
│   - 管理者は Entra ID 管理センターで再処理が可能       │
└────────────────────────────────────────────────────────┘
```

- **割り当て先**: 「割り当て済み」セキュリティグループのみ（動的グループも可）。配布グループは不可。
- **サービスプラン**: ライセンスに含まれる個別のサービス（例: Azure AD Premium P1, Intune）を選択的に有効化/無効化できる。
- **使用場所 (Usage location)**: 法令上の要件で必須。未設定だとライセンス割り当てに失敗するため、グループベースの場合は事前に PowerShell や同期で設定する。

#### よくあるエラーと対処

| エラー | 原因 | 対処 |
|--------|------|------|
| **CountViolation** | テナントの利用可能ライセンス数不足 | ライセンス追加購入、または不要なメンバー削除 |
| **MutuallyExclusiveViolation** | 競合するサービスプラン（例: Exchange の重複） | 競合プランをグループ側で無効化 |
| **ProhibitedInUsageLocationViolation** | ユーザーの使用場所が割り当て不可の国 | 使用場所を変更、または別のライセンスを利用 |
| **UniquenessViolation** | 同じプランを複数経路で受け取っている | Direct 割り当てを解除し、グループ経由のみに統一 |

#### 試験で問われやすいポイント

✅ Entra ID Premium 機能（条件付きアクセス、動的グループ、PIM など）は **ライセンス割り当てが必須**
✅ グループベースライセンスは **「割り当て済み」または「動的ユーザー」セキュリティグループ** に対してのみ可能
✅ ユーザーの **Usage location 未設定** はライセンス割り当て失敗の典型原因
✅ ライセンスエラーが発生したユーザーは Entra ID 管理センターの「ライセンス」→「ライセンスの問題があるユーザー」で確認・再処理する
...
Azure VM のデータは複数の層で保護される。AZ-104 ではそれぞれの位置付けと違いが問われる。

| 暗号化機能 | 暗号化対象 | 既定 | 鍵の管理 | 主な用途 |
|------------|------------|------|----------|----------|
| **Storage Service Encryption (SSE)** | マネージドディスクの **保存データ（プラットフォーム側ストレージ）** | 常時有効・無効化不可 | PMK / CMK (Key Vault) | すべての Azure ディスクの基本暗号化 |
| **Azure Disk Encryption (ADE)** | OS/データディスク内の **OSボリューム単位** (BitLocker / dm-crypt) | 任意 | Key Vault の鍵で BEK を保護 | ゲスト OS 内で暗号化したい場合 |
| **Encryption at host (ホストでの暗号化)** | VM ホスト上の **一時ディスク・OS/データディスクのキャッシュ・エフェメラル OS ディスク** | 既定で無効 | PMK または CMK | ホスト上の I/O キャッシュまで含めたエンドツーエンド暗号化 |
| **Confidential disk encryption** | OS ディスクを **TEE（Trusted Execution Environment）** で保護 | DC/EC v5 系のみ | CMK 必須 | 機密コンピューティング VM |

#### Encryption at host のポイント
...
│ Disk(永続)│ ─────────────→ │ プラットフォーム │
└──────────┘                  └──────────────────┘
   ↑ 一時ディスク / ホストキャッシュは未暗号化

Encryption at host を有効化:
┌──────────┐   暗号化         ┌──────────────────┐
│ Disk(永続)│ ───────────────→│ プラットフォーム │
└──────────┘                  └──────────────────┘
┌─────────────┐                ┌──────────────────┐
...
- **既定では無効**。サブスクリプション単位で `Microsoft.Compute` の機能 `EncryptionAtHost` を有効化したうえで、各 VM 作成時に明示的にオンにする。
- **対応リージョンと VM サイズに制限あり**（`Get-AzComputeResourceSku` で確認可能）。
- **ADE と併用可能**。SSE は無効化できないため、Encryption at host を有効にすると 3層（プラットフォーム/ホスト/ゲスト）すべてで暗号化される構成になる。
- VM が **停止 (deallocated) 状態でないと有効化/無効化できない**。
- CMK を使う場合は **DiskEncryptionSet** を介して Key Vault の鍵を参照する。

#### 試験で問われやすい比較

✅ 「一時ディスクと OS ディスクキャッシュも暗号化したい」→ **Encryption at host**
✅ 「OS 内部の BitLocker / dm-crypt を使いたい」→ **Azure Disk Encryption**
✅ 「保存データを Azure 側で常時暗号化（無効化不可）」→ **Storage Service Encryption**
✅ Encryption at host の有効化は **サブスクリプション機能の登録 → VM 作成時のフラグ指定** の 2 段階
...
Azure Container Apps は、Kubernetes と Dapr / KEDA / Envoy を内包したサーバーレスなコンテナ実行基盤。AKS のような直接的なクラスタ管理を必要とせず、HTTP リクエストやイベントに応じて **0 から自動スケール（スケール to 0 を含む）** ができる。

### ACA / ACI / AKS / App Service の使い分け

| サービス | 抽象度 | スケール | 用途 |
|----------|--------|----------|------|
| **App Service** | 最も高い（PaaS） | 自動スケール（0 不可） | 単一 Web アプリ、API |
| **Container Instances (ACI)** | 単一コンテナ/コンテナグループ | 手動 | 短命なバッチ、CI/CD ジョブ |
| **Container Apps (ACA)** | サーバーレスコンテナ + マイクロサービス | 0 ↔ N 自動スケール（KEDA） | マイクロサービス、イベント駆動 API、バックグラウンドジョブ |
| **AKS** | フル Kubernetes | クラスタ管理者が制御 | Kubernetes ネイティブ機能を直接使いたい場合 |

### Container Apps のコア概念

```
┌─────────────────────────────────────────────┐
│ Environment（環境）                         │
│ ├─ 同一 VNet / Log Analytics を共有          │
│ └─ 1 つの環境内に複数のコンテナアプリ       │
│                                             │
│ ┌──────────────────────────────────────┐    │
│ │ Container App                        │    │
│ │ ├─ Revision（イミュータブルなバージョン)│    │
│ │ │   ├─ Replica（実行中のインスタンス）│    │
│ │ │   │   └─ Container（Pod 内コンテナ）│    │
│ │ ├─ Ingress (HTTP/TCP)                │    │
│ │ ├─ Scale rules (HTTP / KEDA / CPU)   │    │
│ │ └─ Dapr sidecar（任意）              │    │
│ └──────────────────────────────────────┘    │
└─────────────────────────────────────────────┘
```

- **Revision**: コンテナアプリの不変スナップショット。新しいデプロイは新リビジョンとして作成され、トラフィック分割（Blue/Green、A/B、カナリア）に利用する。
- **Ingress**: 外部公開（External）または環境内のみ（Internal）。HTTPS は自動付与され、独自ドメインも対応。
- **Scale rules**:
  - HTTP 同時接続数（既定）
  - CPU/メモリ
  - KEDA ベースのイベントトリガー（Azure Service Bus, Event Hubs, Storage Queue, Kafka など）
  - レプリカ最小数 0（**スケール to 0**）から最大値まで指定可能
- **Dapr 統合**: `state`, `pubsub`, `bindings`, `secrets` の標準ビルディングブロックを sidecar として利用。
- **VNet 統合**: 環境作成時に既存サブネットを指定。Internal 環境はプライベートな Load Balancer として公開される。

### よくあるユースケース

- **イベント駆動のマイクロサービス**: キューにメッセージが投入されると自動でスケールアウト、空になればゼロにスケールイン。
- **バックグラウンドジョブ / Worker**: HTTP を持たないリビジョンも作成可能。
- **既存コンテナの段階的モダナイズ**: AKS まで踏み込まなくても、ACI より高度な機能（Revisions / Dapr / KEDA）を使いたい場合。

### 試験で問われやすい比較

✅ **0 から自動スケール**できるコンテナサービス → Container Apps
✅ **Kubernetes API** を直接使いたい → AKS
✅ **単発のコンテナ実行**（バッチ等） → ACI
✅ Container Apps の **Revision** はトラフィック分割と Blue/Green に必須概念

## 3.8 Bicep ファイルとARMテンプレート

### 概念説明

Bicep は ARM テンプレート（JSON）の **DSL（ドメイン特化言語）**。同じリソースマネージャー API を呼び出すが、より宣言的で読みやすい構文を提供する。Bicep ファイル (`.bicep`) は Azure CLI / PowerShell によって ARM JSON に **トランスパイル** されてからデプロイされる。

### Bicep と ARM JSON の対応
...
// 同等の ARM JSON（抜粋）
{
  ...
  "outputs": {
    "storageId": { "type": "string", "value": "[resourceId('Microsoft.Storage/storageAccounts', parameters('storageAccountName'))]" }
  }
}
```
...
| 要素 | キーワード | 例 | 用途 |
|------|------------|----|------|
| パラメータ | `param` | `param env string = 'dev'` | デプロイ時の入力 |
| 変数 | `var` | `var prefix = 'app-${env}'` | テンプレート内の計算値 |
| リソース | `resource` | 上記参照 | Azure リソースの宣言 |
| 既存リソース参照 | `resource ... existing = { ... }` | `resource kv 'Microsoft.KeyVault/vaults@...' existing = { name: 'mykv' }` | 既存リソースの参照のみ |
| 出力 | `output` | `output id string = sa.id` | 後続デプロイへの値の引き渡し |
| モジュール | `module` | `module net 'modules/network.bicep' = {...}` | テンプレートの分割と再利用 |
| 条件 | `if (...)` | `resource pip '...' = if (createPip) { ... }` | 条件付きデプロイ |
| ループ | `for` | `[for i in range(0, count): { ... }]` | 配列によるリソース複製 |

### ARM → Bicep への変換ワークフロー
...
```bash
# ARM JSON を Bicep に変換（脱コンパイル）
az bicep decompile --file existing-template.json

# Bicep を ARM JSON にビルド（手動でも可。デプロイ時は自動）
az bicep build --file main.bicep

# Bicep ファイルのデプロイ（直接デプロイ可能）
az deployment group create \
  --resource-group rg-prod \
  --template-file main.bicep \
  --parameters env=prod

# パラメータファイル (.bicepparam または .json) を使ったデプロイ
az deployment group create \
  --resource-group rg-prod \
  --template-file main.bicep \
...
- **API バージョン**: `Microsoft.Storage/storageAccounts@2023-01-01` のように **タイプ@バージョン** で指定。古いテンプレートを修正する際は API バージョンを最新に上げると新機能（例: `minimumTlsVersion`）が利用可能になる。
- **シンボリック名**: `resource sa '...' = { ... }` の `sa` がリソースの参照名。`output` や `dependsOn` で利用。
- **暗黙の依存関係**: 別リソースのプロパティ（`sa.id`）を参照すると **自動で `dependsOn` が解決される**。手動で書く必要があるのは循環参照を避けたい特殊ケースのみ。
- **モジュール**: 大規模テンプレートは `modules/network.bicep` のように分割し、`module` キーワードで呼び出す。サブスクリプション・管理グループ・テナントスコープのデプロイにも対応。
- **decompile の限界**: ARM の関数や `copy` ループの一部は手作業の整形が必要。decompile 後は **`az bicep build` で再 JSON 化して差分が無いか確認** するのが定石。

### 試験で問われやすいポイント

✅ Bicep は **ARM テンプレートのトランスパイラ** であり、最終的なデプロイは ARM API
✅ **`az bicep decompile`** が ARM JSON → Bicep の変換コマンド
✅ プロパティ参照（`sa.id` 等）で **依存関係は自動解決**
✅ **`what-if`** で実際にデプロイする前に変更プレビューが可能
✅ パラメータファイルは `.json` のほか **`.bicepparam`** を使うと型付きで安全
...
Azure Bastion は、**ブラウザ越しに RDP / SSH で VM へ接続できるマネージド PaaS**。VM にパブリック IP を割り当てなくても、Azure Portal から TLS 経由で安全にリモート接続できる。

```
従来:
  管理者 ──RDP/SSH──→ VM(パブリックIP) ←攻撃面
  
Bastion:
  管理者 ──HTTPS(443)──→ Azure Portal ──→ Bastion(VNet内) ──→ VM(プライベートIP)
                                            ↑ TLSターミネート
```

### Bastion のメリット

- **VM にパブリック IP が不要** → ブルートフォース等の攻撃面を縮小
- **NSG で 3389/22 をインターネットに開放しない** で済む
- **Azure Portal / ネイティブクライアント** から接続可能（ネイティブ接続は Standard 以上）
- **MFA / 条件付きアクセス** を Entra ID 経由で適用可能

### Bastion SKU

| SKU | 主な機能 | 用途 |
|-----|----------|------|
| **Developer** | 同一サブネット内の単一 VM へのみ接続 (1 ユーザー) | 個人開発・検証 |
| **Basic** | RDP/SSH（ポータルのみ）、ピアリング VNet 接続 | 小規模本番 |
| **Standard** | ネイティブクライアント (`az network bastion ssh`)、ファイル転送、VM スケールセット接続、UDR、シェアブルリンク | 一般本番 |
| **Premium** | プライベートのみのデプロイ、セッション記録、ZTNA 強化 | コンプライアンス要件が高い環境 |

### 必須要件

```
┌──────────────────────────────────────────────────────┐
│ VNet 内に「AzureBastionSubnet」という名前のサブネット │
│ ├─ 名前は完全に一致しなければならない                │
│ ├─ 最小サイズは /26 以上（Standard 以上はスケール用）│
│ ├─ NSG を関連付ける場合は 必要なポート許可が必須:    │
│ │    - Inbound 443 (GatewayManager, Internet)        │
│ │    - Outbound 22/3389 (VirtualNetwork)             │
│ │    - Outbound 443 (AzureCloud)                     │
│ └─ ルートテーブルで 0.0.0.0/0 を NVA 等に向けない    │
└──────────────────────────────────────────────────────┘
```

### ピアリング VNet 経由の接続

Bastion はハブ VNet に 1 台デプロイし、**VNet ピアリング** で接続されたスポーク VNet 内の VM にも接続できる（ハブアンドスポーク構成での標準パターン）。Developer SKU は同一 VNet 内のみ。

### 接続方法

```bash
# ポータル: VM 一覧 → 接続 → Bastion タブ → ユーザー名/SSHキーで接続

# Standard 以上: Azure CLI ネイティブクライアント
...
✅ サブネット名は **`AzureBastionSubnet`（厳密一致、最小 /26）**
✅ VM に **パブリック IP は不要**（プライベート IP で接続）
✅ **ピアリングで接続された VNet** 内の VM にも到達可能（Basic 以上）
✅ **ネイティブ SSH/RDP クライアント / ファイル転送** は **Standard SKU** から
✅ Bastion 自体には **Standard SKU のパブリック IP** が必須
...
### Vault の種類: Recovery Services vault と Backup vault

Azure Backup の保管先（Vault）には **2 種類** あり、対象ワークロードと提供機能が異なる。AZ-104 ではどちらを使うべきかが頻出。

| 観点 | **Recovery Services vault (RSV)** | **Backup vault** |
|------|-----------------------------------|------------------|
| 登場時期 | 旧来からの Vault | 新世代の Vault |
| 主な対象ワークロード | Azure VM、Azure Files、SQL/SAP HANA in VM、MARS（オンプレ）、Azure Backup Server / DPM、**Azure Site Recovery (ASR)** | Azure Disks、Azure Blob (オペレーショナル/ボールト保護)、PostgreSQL (Flexible Server / 単一サーバー)、AKS、**Azure Files (ボールト保護層)** |
| ASR | **対応** | 非対応 |
| ストレージレプリケーション | LRS / GRS / **ZRS** | LRS / GRS / **ZRS** |
| 不変ヴォールト (Immutable vault) | 対応 | 対応 |
| 多要素認証保護 (MUA) | 対応 | 対応 |
| クロスリージョンリストア | GRS の場合に対応 | 一部ワークロード（PostgreSQL 等）で対応 |
| 管理 API | `Microsoft.RecoveryServices` | `Microsoft.DataProtection` |
...
│ 何を保護したい？                                        │
│                                                         │
│  Azure VM / Azure Files (ファイル共有スナップショット)  │
│   ├─ ASR も使う ─────────→ Recovery Services vault      │
│   └─ Backup のみ ────────→ Recovery Services vault      │
│                                                         │
│  オンプレミス (MARS / DPM / Azure Backup Server)        │
│   └────────────────────→ Recovery Services vault        │
│                                                         │
│  Azure Disks / Blob / PostgreSQL / AKS                  │
│   └────────────────────→ Backup vault                   │
│                                                         │
│  Azure Files をボールト保護層で長期保管                 │
│   └────────────────────→ Backup vault                   │
└─────────────────────────────────────────────────────────┘
```
...
- **論理削除 (Soft delete)**: 既定で 14 日間（最大 180 日に拡張可）。誤削除されたバックアップ項目を回復可能。
- **不変ヴォールト**: 一度有効化すると保持期間の短縮や削除をブロックする（Immutable）。コンプライアンス要件で使用。
- **MUA (Multi-User Authorization)**: 重要操作（保持期間短縮、削除等）に Resource Guard 経由で第三者承認を要求。
...
## 5.8 Azure Network Watcher

### 概念説明

Azure Network Watcher は、**Azure ネットワークの監視・診断・ロギングをまとめて提供するサービス**。リージョンごとに 1 つ自動的に有効化される（手動でも作成可）。

### 主要ツール

| ツール | 何ができる | ユースケース |
|--------|------------|--------------|
| **IP Flow Verify** | 5 タプル（送信元/宛先 IP・ポート・プロトコル）について **NSG により許可/拒否されるかを判定** | 「なぜこの通信が遮断されているか」 |
| **NSG Diagnostic** | 同様に経路上で **適用される NSG ルール一覧** を可視化 | 多段 NSG 環境のトラブルシュート |
| **Next Hop** | 指定 IP 宛のパケットの **次ホップ（VNet ローカル / インターネット / 仮想アプライアンス / なし）** を確認 | UDR / NVA の検証 |
| **Effective Security Rules** | NIC に **実効的に適用されている NSG ルール** を集約表示 | 複合 NSG の整合性確認 |
| **Connection Troubleshoot** | **送信元 → 宛先** へのテスト接続を実行し、ホップ・遅延・失敗理由を返す | アドホックな疎通確認 |
| **Connection Monitor** | 送信元（VM/VMSS/Bastion など）から複数宛先へ **継続的に接続状態を監視**。メトリックとログ化 | SLA 監視、ハイブリッド接続の常時監視 |
| **Packet Capture** | VM のネットワークインターフェースで **パケットキャプチャ**。Storage / ローカルに保存 | 詳細プロトコル解析 |
| **VPN Troubleshoot** | VPN ゲートウェイ / 接続のヘルス診断 | サイト間 VPN の問題切り分け |
| **Topology** | リソースグループ単位の **ネットワーク構成図** を生成 | ドキュメント / レビュー |
| **NSG Flow Logs** | NSG を通過するトラフィックを **JSON で Storage に記録**（v1/v2）。Traffic Analytics と連携 | 監査・脅威検知 |
| **VNet Flow Logs (新)** | VNet 単位でフローログを取得（NSG に依存しない、より新しい方式） | NSG レス構成や AVNM 環境 |

### Connection Monitor の構造
...
- ハイブリッド（オンプレ ←→ Azure）監視には **Network Watcher Agent VM 拡張機能** または **Log Analytics エージェント** を送信元に導入。
- メトリック化されるため、**メトリックアラート**で SLA 違反を通知可能。
...
NSG Flow Logs (v2)
   ↓ Storage Account
   ↓
Traffic Analytics (Log Analytics + 機械学習)
   ↓
- 不正な通信パターンの検出
- アプリケーション通信トポロジの可視化
- リージョンを跨いだトラフィックの集計
...
- **保持期間** は Storage Account のライフサイクルで管理。
- Traffic Analytics は **追加課金** だが、生 JSON を読まずにダッシュボードで分析できる。

### 試験で問われやすいポイント

✅ 「特定の通信が NSG で遮断されているか確認したい」→ **IP Flow Verify**
✅ 「VPN・ExpressRoute・拠点間の **継続的な疎通監視**」→ **Connection Monitor**
✅ 「ある時点での疎通テスト」→ **Connection Troubleshoot**
✅ 「NIC で実際に有効な NSG ルール一覧」→ **Effective Security Rules**
✅ 「NSG を通過したフローを長期保管・分析」→ **NSG Flow Logs + Traffic Analytics**
✅ NSG Flow Logs の保存先は **Storage Account**（Log Analytics ではない）

## 5.9 Azure Monitor Insights (VM / Storage / Network)

### 概念説明

Azure Monitor の **Insights** は、対象リソース向けに事前構成されたダッシュボード・メトリック収集・分析パッケージ。各 Insights を有効化することで、KQL を書かずとも標準的な観測がすぐに利用できる。

### VM Insights

- **対象**: Azure VM、VMSS、Arc 対応サーバー、オンプレ VM。
- **収集内容**:
  - パフォーマンスメトリック（CPU・メモリ・ディスク・ネットワーク）
  - **マップ機能** によるプロセス間の依存関係（同一 VM 内 / 他 VM・外部サービス）
- **アーキテクチャ**: **Azure Monitor Agent (AMA) + Data Collection Rule (DCR)** で収集。旧来の Log Analytics エージェント (MMA) は 2024 年に廃止済み。
- **必要なリソース**: Log Analytics ワークスペース + DCR。

```
Azure VM ──(AMA)──→ DCR ──→ Log Analytics
                              ├─ Performance テーブル
                              ├─ InsightsMetrics テーブル
                              └─ VMConnection / VMProcess テーブル
                                    ↓
                                VM Insights ブック
```
...
- **対象**: ストレージアカウント（Blob / File / Queue / Table）。
- **提供内容**:
  - 容量、トランザクション、エラーレート、可用性、レイテンシの集約ダッシュボード
  - 複数アカウントを横断するワークブック
- **データソース**: 既定の **プラットフォームメトリック**（追加コストなし） + 任意の **診断設定** によるリソースログ。

### Network Insights

- **対象**: VNet ゲートウェイ、Application Gateway、Load Balancer、ExpressRoute、Front Door、NIC、Public IP など Azure ネットワークリソース全般。
- **提供内容**:
  - リソース別ダッシュボード（**接続性・トラフィック・エラー**の標準ビュー）
  - **Connection Monitor** や **NSG Flow Logs / Traffic Analytics** とのシームレスな連携
  - トポロジビューでネットワークリソース間の依存を可視化

### 3つの Insights の使い分け

| 何を見たい？ | 使う Insights |
|---------------|---------------|
| VM 上のプロセス・依存関係・パフォーマンス | **VM Insights** |
| ストレージアカウントの容量と I/O パターン | **Storage Insights** |
| ネットワークリソース横断のヘルスとトラフィック | **Network Insights** |

### 共通の運用ポイント

- すべての Insights は **Log Analytics ワークスペース** がバックエンド。コストは取り込みデータ量に比例する。
- 既存のリソースに **後付けで有効化** できる。Insights を有効化しても元のリソース設定は変更されない。
- カスタマイズしたい場合は、Insights が提供する **ブック (Workbook)** を保存・複製して KQL を書き換える。

### 試験で問われやすいポイント

✅ VM Insights は **AMA + DCR** が前提（旧 MMA は廃止）
✅ Insights のバックエンドは **Log Analytics ワークスペース**（コストは取り込みデータ量）
✅ 「プロセスの依存関係を見たい」→ **VM Insights のマップ**
✅ 「ストレージアカウントの容量・トランザクションをまとめて見たい」→ **Storage Insights**
✅ 「Application Gateway / VPN GW / Load Balancer のヘルスを横断確認」→ **Network Insights**
